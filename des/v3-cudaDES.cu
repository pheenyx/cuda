...
__global__ void DESkernel(des_context ctx, const unsigned char* key, const unsigned char* plain, unsigned char* cipher);
...
__device__ int des_crypt_ecb_cuda( des_context *ctx, const unsigned char input[8], unsigned char output[8] );
...
__global__ void DESkernel(des_context* ctx, const unsigned char* key, const unsigned char* plain, unsigned char* cipher)
{
    ...
    des_crypt_ecb_cuda(ctx, plain, cipher);
}
...
{   
    ...
    //invoke kernel
    int N = 1;
    DESkernel<<<1, N, 3*real_size>>>(d_ctx, d_key, d_plain, d_cipher);
...
*/    printf("\n");
    ...
    if(isSerial == 0)
    {
        ...
        cudaFunction(*ctx,my_keys,my_plain[0],buf,8);
        //
    }
    else
    {
